...
app = modal.App("MelodyX-music-generator")
...
melodyx_secret = modal.Secret.from_name("melodyx-secret")
...
class GenerateMusicResponseS3(BaseModel):
    s3_key: str             # Location of the song in the bucket
    cover_image_s3_key: str           # Location of the thumbnail image in the bucket
    categories: List[str]
...
class MusicGenServer:
    ...
    def generate_categories(self, description: str) -> List[str]:
        prompt = f"Based on the following music description, list 3-5 relevant genres or categories as a comma-separated list. For example: Pop, Electronic, Sad, 80s, romantic. Description: '{description}'"
    ...
    def generate(self) -> GenerateMusicResponse:
        ...
        # converts raw binary audio data (audio_bytes) into a base64-encoded string that can be safely embedded in JSON API responses for web frontends to decode and play as audio.
    ...
    # Generating lyrics
        ...
        return self.generate_and_upload_s3(prompt=prompt, lyrics=lyrics,
    ...
    def generate_with_lyrics(self, request: GenerateWithCustomLyricsRequest) -> GenerateMusicResponseS3:
        return self.generate_and_upload_s3(prompt=request.prompt, lyrics=request.lyrics,
                                           description_for_categorization=request.prompt,
                                           **request.model_dump(exclude={"prompt", "lyrics"}))

    @modal.fastapi_endpoint(method="POST")
    def generate_with_decribed_lyrics(self, request: GenerateWithDescribedLyricsRequest) -> GenerateMusicResponseS3:
        # Generating lyrics
        lyrics = ""
        if not request.instrumental:
            lyrics = self.generate_lyrics(request.described_lyrics)
        return self.generate_and_upload_s3(prompt=request.prompt, lyrics=lyrics,
                                           description_for_categorization=request.prompt,
...
def main():
    ...
    # will get us the url of the endpoint we created (@modal.fastapi_endpoint())
    endpoint_url = server.generate_with_decribed_lyrics.get_web_url()

    request_data = GenerateWithDescribedLyricsRequest(
        prompt="rave, funk, 140BPM, disco",
        described_lyrics="lyrics on heartbreak",
        guidance_scale=15
    )

    headers = {
        "Authorization": "Bearer as-DyEN0VF4CfrDWA2AQ3rnS3",
...
# Note : My melodyx-secret contains S3_BUCKET_NAME + IAM credentials (from my music-generation-backend-policy).
# S3 = My music library backbone! Without it, generated tracks vanish forever
